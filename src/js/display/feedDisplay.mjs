 ...
 * Posts data being sent from another function
 * @param {object} posts Object containing an array of blog posts data and information about the blog posts
 */
export async function displayFeed(posts) {
  ...
  postData.forEach((post) => {
    ...
    infoBar.title = `Go to ${post.author.name}'s profile`;
    ...
    ddMenyLiUpdate.addEventListener("click", function () {
      updatePostModal.addEventListener("shown.bs.modal", function () {
        ...
        handlers.setUpdatePostForm(postId);
      });
    });
    ...
    // function to delete users post inside dropdown menu
    // ddMenyLiUpdateA.addEventListener("click", function () {
    ...
    for (const reaction of post.reactions) {
      if (reaction.reactors && reaction.reactors.length > 0) {
        const isReacting = reaction.reactors.some((reactor) => {
          if (reactor === username) {
            return true;
          }
        });
